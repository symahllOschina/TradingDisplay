...
public class MainFragment extends BrowseFragment {
    ...
    private void loadRows() {
        //Movie一个实现了序列化的Bean。setupMovies（）是产生movie数据的方法
        ...
        for (i = 0; i < NUM_ROWS; i++) {
            ...
            //里面要传一个Persenter类型的，Persenter里面有三个抽象方法，复写了Adapter
            ArrayObjectAdapter listRowAdapter = new ArrayObjectAdapter(cardPresenter);
            for (int j = 0; j < NUM_COLS; j++) {
                //添加Movie，产生页面右边的15个数据
                listRowAdapter.add(list.get(j % 5));
            }
            //产生左边的标题数据
            HeaderItem header = new HeaderItem(i, MovieList.MOVIE_CATEGORY[i]);
            //listRowAdapter是页面右边的数据  new ListRow(header, listRowAdapter)这里绑定左边主标题和右边子数据
    ...
    /**
     * setHeadersTransitionOnBackEnabled(false); //这里是设置左边引导（即菜单导航）的显示与否。
